...
plot_cell_clusters(cde_SC, 1, 2, color = "Cluster", markers = c("Aldh1a2")) + facet_wrap(~Time_point)
...
plot_cell_trajectory(cde_SC, markers = c("Vcam1"), use_color_gradient = TRUE)
...
#Get all clusters with expression below threshold
# Note: Manually check average expressions
Adventi_subsets <- Adventi.classificator[Adventi.classificator <= 3.5]
Adventi_subsets
nonAdventi_subsets <- Adventi.classificator[Adventi.classificator > 3.5]
nonAdventi_subsets
...
plot_cell_clusters(cde_Adventi, 1, 2, markers = c("Tnfsf11")) + facet_wrap(~State)
...
#Save/Load R object
#####
saveRDS(cde_SC, file="~/NAS2/pezoldt/Analysis/scRNAseq/monocle/20180926_d0_d10_SC.Rds") 
saveRDS(cde_Adventi, file="~/NAS2/pezoldt/Analysis/scRNAseq/monocle/20180926_d0_d10_Adventi.Rds")
saveRDS(cde_nonAdventi, file="~/NAS2/pezoldt/Analysis/scRNAseq/monocle/20180926_d0_d10_nonAdventi.Rds")
saveRDS(cde_PvC, file="~/NAS2/pezoldt/Analysis/scRNAseq/monocle/20180926_d0_d10_PvC.Rds")
saveRDS(cde_Endothelial, file="~/NAS2/pezoldt/Analysis/scRNAseq/monocle/20180926_d0_d10_Endothelial.Rds")
cde_nonAdventi <- readRDS(file="~/NAS2/pezoldt/Analysis/scRNAseq/monocle/20180826_d0_d10_nonAdventi.Rds")
cde_SC <- readRDS(file="~/NAS2/pezoldt/Analysis/scRNAseq/monocle/20180826_d0_d10_SC.Rds")
...
plot_genes_in_pseudotime(cde_x_subset, color_by = "Time_point")
...
#Order cells
#cde_SC <- orderCells(cde_SC)
#cde_SC <- orderCells(cde_x, root_state = GM_state(cde_x))
#
plot_cell_trajectory(cde_x, color_by = "Cluster")
plot_cell_trajectory(cde_x, color_by = "Pseudotime")
...
plot_genes_branched_heatmap(cde_x[row.names(subset(BEAM_res,
                                                  qval < 1e-10)),],
                            branch_point = 1,
                            num_clusters = length(levels(as.factor(pData(cde_x)$Cluster))),
                            cores = 8,
...
cde_x_genes <- row.names(subset(fData(cde_x),
                               gene_short_name %in% c("Acta2", "Irf1", "Ccl2",
                                                      "Fmod","Nfkbiz")))
plot_genes_branched_pseudotime(cde_x[cde_x_genes,],
                               branch_point = 1,
                               color_by = "Cluster",
                               ncol = 1,
                               min_expr = 1,
...
#Plot cohort of genes
full_model_fits <- fitModel(cde_x[sample(nrow(fData(cde_x)), 100),],  
                            modelFormulaStr="~sm.ns(Pseudotime)")

expression_curve_matrix <- responseMatrix(full_model_fits)
clusters <- clusterGenes(expression_curve_matrix, k=9)
plot_clusters(cde_x[ordering_genes,], clusters)
