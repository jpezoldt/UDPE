...
obs.list_minus <- readRDS(paste(PATH_Output,"/obs.list_minus.Rds", sep=""))
#####
...
#Scale
mLN_Ontogeny_minus.merged <- ScaleData(object = mLN_Ontogeny_minus.merged, vars.to.regress = c("S.Score", "G2M.Score","nGene", "percent.mito","tech"), 
...
for (i in 1:length(obs.list_minus)) {
  genes.use <- c(genes.use, head(rownames(obs.list_minus[[i]]@hvg.info), 1000))
}
genes.use <- names(which(table(genes.use) > 1))
for (i in 1:length(obs.list_minus)) {
...
saveRDS(mLN_Ontogeny_minus.merged, paste(PATH_Output,"/MultiAlign_20_1.1_regressed_tech_all_D0_D10_D24_D56_D300.Rds", sep=""))

mLN_Ontogeny_minus.merged <- readRDS(paste(PATH_Output,"/MultiAlign_20_1.2_regressed_all_D0_D10_D24_D56_D300.Rds", sep=""))
#Change cluster ID
...
mLN_Ontogeny_minus.merged <- CellCycleScoring(object = mLN_Ontogeny_minus.merged, s.genes = s.genes, g2m.genes = g2m.genes, 
                           set.ident = TRUE)
head(x = mLN_Ontogeny_minus.merged@meta.data)
...
#Run PCA on the 
mLN_Ontogeny_minus.merged <- RunPCA(object = mLN_Ontogeny_minus.merged, pc.genes = c(s.genes, g2m.genes), do.print = FALSE)
PCAPlot(mLN_Ontogeny_minus.merged)

mLN_Ontogeny_minus.merged <- ScaleData(object = mLN_Ontogeny_minus.merged, vars.to.regress = c("S.Score", "G2M.Score"), 
                    display.progress = FALSE)

# Now, a PCA on the variable genes no longer returns components associated
# with cell cycle
mLN_Ontogeny_minus.merged <- RunPCA(object = mLN_Ontogeny_minus.merged, pc.genes = marrow@var.genes, genes.print = 10)
PCAPlot(mLN_Ontogeny_minus.merged)
...
pdf(paste(PATH_Output,"/MultiAlign_20_1.2_regressed_all_NC_Top40_D0_D10_D24_D56_D300.pdf", sep=""))
...
#Save DEGs
#write.table(mLN_Ontogeny_minus.merged.markers, paste(PATH_Output,"/MultiAlign_20_1.1_regressed_all_DEGs_D0_D10_D24_D56_D300.csv", sep=""), dec=".", sep=",")
mLN_Ontogeny_minus.merged.markers <- read.table(paste(PATH_Output,"/MultiAlign_20_1.2_regressed_all_DEGs_D0_D10_D24_D56_D300.csv", sep=""), dec=".", sep=",")
...
#Plot differential expression of Marker genes
title <- "Dim12_Res1.2_Dotplot_Top10_D0_10_24_56_300"
...
l_cells_per_condition <- lapply(l_condition, function(x) {
  #x <- l_condition[[1]]
  x_cells <- unlist(lapply(split(x, x$res.1.2), nrow))
  x_cells
})
