...
#load seurat object
seurats.integrated <- readRDS(file=paste(output_PATH_RDS,"/Tx_LN.integrated.Rds",sep=""))
...
sample_3 <- "Tx_mLN"
sample_4 <- "Tx_pLN"
all_samples_names <- c(sample_2,sample_3, sample_4) #sample_1,  
#list of all seurat objects
l_data_raw <- list(sample_2.data,sample_3.data, sample_4.data) #sample_1.data,
...
seurats.integrated <- FindClusters(seurats.integrated, reduction.type = "cca",
                                    dims.use = 1:16, save.SNN = T, resolution = 0.8)
seurats.integrated <- RunTSNE(seurats.integrated,
                               reduction.use = "cca",
...
seurat_1 <- l_seurats[[1]]
seurat_2 <- l_seurats[[2]]
seurat_nonEC_SC.merged <- MergeSeurat(seurat_1, seurat_2, do.normalize = FALSE)
#if more than two seurat objects
if(length(l_seurats) > 2){
  for(i in 1:(length(l_seurats)-2)){
    #take 2 +ith element and append to merged seurat object
    k = i + 2
    seurat_i <- l_seurats[[k]]
    seurat_nonEC_SC.merged <- MergeSeurat(seurat_nonEC_SC.merged, seurat_i, do.normalize = FALSE)
  }
}

#Eliminate mitochondrial high cells and dublets
#Identify cells with mitochondrial readss
mito.genes <- grep(pattern = "^mt-", x = rownames(seurat_nonEC_SC.merged@data), value = TRUE)
percent.mito <- colSums(as.matrix(seurat_nonEC_SC.merged@raw.data[mito.genes, ]))/colSums(as.matrix(seurat_nonEC_SC.merged@raw.data))

# AddMetaData adds columns to object@data.info and good to store QC stats
seurat_nonEC_SC.merged <- AddMetaData(object = seurat_nonEC_SC.merged, metadata = percent.mito, col.name = "percent.mito")
VlnPlot(object = seurat_nonEC_SC.merged, features.plot = c("nGene", "nUMI", "percent.mito"), nCol = 3)
par(mfrow = c(1, 2))
GenePlot(object = seurat_nonEC_SC.merged, gene1 = "nUMI", gene2 = "percent.mito")
GenePlot(object = seurat_nonEC_SC.merged, gene1 = "nUMI", gene2 = "nGene")
#Filter cells
seurat_nonEC_SC.merged <- FilterCells(object = seurat_nonEC_SC.merged, subset.names = c("nGene", "percent.mito"), 
                                     low.thresholds = c(min_gene_number, -Inf), high.thresholds = c(nGene, mito_cutoff),
                                     cells.use = seurats.integrated_NO_LECs_BECs)
#Number of cells
print(paste("Number of non-endothelial SC: ", nrow(seurat_nonEC_SC.merged@meta.data), sep = ""))

#Normalize
seurat_nonEC_SC.merged <- NormalizeData(seurat_nonEC_SC.merged)
seurat_nonEC_SC.merged <- FindVariableGenes(seurat_nonEC_SC.merged, do.plot = F, display.progress = F)
seurat_nonEC_SC.merged <- ScaleData(seurat_nonEC_SC.merged)
#Run PCA
seurat_nonEC_SC.merged <- RunPCA(object = seurat_nonEC_SC.merged, pc.genes = seurat_nonEC_SC.merged@var.genes, do.print = TRUE, pcs.print = 1:10, 
                       genes.print = 10)

# Examine and visualize PCA results
PrintPCA(object = seurat_nonEC_SC.merged, pcs.print = 1:5, genes.print = 5, use.full = FALSE)
VizPCA(object = seurat_nonEC_SC.merged, pcs.use = 1:6)
PCAPlot(object = seurat_nonEC_SC.merged, dim.1 = 1, dim.2 = 2)

#Setup list of QCed samples
l_ob_nonEC_SC <- list()
for(i in 1:length(l_seurats)){
  temp_seurat <- l_seurats[[i]]
  #Identify cells with mitochondrial readss
...
# Run multi-set CCA
seurat_nonEC_SC.integrated <- RunMultiCCA(l_ob_nonEC_SC, genes.use = genes.use, num.ccs = dims_use)

# CC Selection
MetageneBicorPlot(seurat_nonEC_SC.integrated, grouping.var = "tech", dims.eval = 1:15)

# Run rare non-overlapping filtering
seurat_nonEC_SC.integrated <- CalcVarExpRatio(object = seurat_nonEC_SC.integrated, reduction.type = "pca",
                                    grouping.var = "tech", dims.use = 1:dims_use)
seurat_nonEC_SC.integrated <- SubsetData(seurat_nonEC_SC.integrated, subset.name = "var.ratio.pca",
                               accept.low = 0.5)

# Alignment
seurat_nonEC_SC.integrated <- AlignSubspace(seurat_nonEC_SC.integrated,
                                  reduction.type = "cca",
                                  grouping.var = "tech",
                                  dims.align = 1:16)

# t-SNE and Clustering
seurat_nonEC_SC.integrated <- FindClusters(seurat_nonEC_SC.integrated, reduction.type = "cca",
                                 dims.use = 1:16, save.SNN = T, resolution = 0.8)
seurat_nonEC_SC.integrated <- RunTSNE(seurat_nonEC_SC.integrated,
                            reduction.use = "cca",
                            dims.use = 1:16, save.SNN = T,
                            resolution = resolution_merge, force.recalc = TRUE)
...
#Check allocation of cells
#sample_1_cells <- grep(pattern = "^sample_1_", x = rownames(seurat_nonEC_SC.integrated@meta.data), value = TRUE)
sample_2_cells <- grep(pattern = "^sample_2_", x = rownames(seurat_nonEC_SC.integrated@meta.data), value = TRUE)
sample_3_cells <- grep(pattern = "^sample_3_", x = rownames(seurat_nonEC_SC.integrated@meta.data), value = TRUE)
sample_4_cells <- grep(pattern = "^sample_4_", x = rownames(seurat_nonEC_SC.integrated@meta.data), value = TRUE)
#p1 <- TSNEPlot(seurat_nonEC_SC.integrated, group.by = "tech", do.return = T, pt.size = 0.4, cells.use = sample_1_cells)
p2 <- TSNEPlot(seurat_nonEC_SC.integrated, group.by = "tech", do.return = T, pt.size = 0.4, cells.use = sample_2_cells)
p3 <- TSNEPlot(seurat_nonEC_SC.integrated, group.by = "tech", do.return = T, pt.size = 0.4, cells.use = sample_3_cells)
p4 <- TSNEPlot(seurat_nonEC_SC.integrated, group.by = "tech", do.return = T, pt.size = 0.4, cells.use = sample_4_cells)
p5 <- TSNEPlot(seurat_nonEC_SC.integrated, do.return = T, pt.size = 0.2, do.label = FALSE)
#test <- LN@meta.data[order(-LN@meta.data$),]
p6 <- TSNEPlot(seurat_nonEC_SC.integrated, group.by = "tech", do.return = T, pt.size = 0.2,
               colors.use = c("orange","green","blue","purple"))
par(mfrow = c(3, 2))
plot_grid(p2, p3, p4, p5, p6)


FeaturePlot(seurat_nonEC_SC.integrated, features.plot = c("Pdpn","Pecam1","Lyve1","Ackr4", 
                                                "Tnfsf11","Aldh1a3","Ptgis","Ccl19",
                                                "Madcam1","Cd34","Tnfsf13b","Il6", 
                                                "Il6","Ackr3","Ly6c1","Col4a1"),
            min.cutoff = "q9",
            cols.use = c('lightgrey', 'brown'),
            pt.size = 0.2)

#Save Seurat Object
saveRDS(seurat_nonEC_SC.integrated, file=paste(output_PATH_RDS,"/Tx_LN_nonEC_SC.integrated.Rds",sep=""))

######
#Select Adventitial cells
#####
#Identify LECs and BECs
VlnPlot(object = seurat_nonEC_SC.integrated, features.plot = c("Cd34"))
seurat_nonEC_SC.integrated_aExp <- AverageExpression(seurat_nonEC_SC.integrated)
aExp_Cd34 <- subset(seurat_nonEC_SC.integrated_aExp, rownames(seurat_nonEC_SC.integrated_aExp) == c("Cd34"))
#Grab colnames/clusters that have Pecam1 expression < 1 on average per cluster
cluster_keep <- as.numeric(rownames(subset(as.data.frame(t(aExp_Cd34)), Cd34 > 3)))

#Get cells that are not within the cluster BEC and LEC cluster
all_cells <- seurat_nonEC_SC.integrated@meta.data
all_cells_keep <- subset(all_cells, res.0.8 == 1000)
...
seurat_nonEC_SC.integrated_ONLY_Adventitial <- as.character(rownames(all_cells_keep))
...
seurat_1 <- l_seurats[[1]]
seurat_2 <- l_seurats[[2]]
seurat_nonEC_advSC.merged <- MergeSeurat(seurat_1, seurat_2, do.normalize = FALSE)
#if more than two seurat objects
if(length(l_seurats) > 2){
  for(i in 1:(length(l_seurats)-2)){
    #take 2 +ith element and append to merged seurat object
    k = i + 2
    seurat_i <- l_seurats[[k]]
    seurat_nonEC_SC.merged <- MergeSeurat(seurat_nonEC_advSC.merged, seurat_i, do.normalize = FALSE)
  }
}

#Eliminate mitochondrial high cells and dublets
#Identify cells with mitochondrial readss
mito.genes <- grep(pattern = "^mt-", x = rownames(seurat_nonEC_advSC.merged@data), value = TRUE)
percent.mito <- colSums(as.matrix(seurat_nonEC_advSC.merged@raw.data[mito.genes, ]))/colSums(as.matrix(seurat_nonEC_advSC.merged@raw.data))

# AddMetaData adds columns to object@data.info and good to store QC stats
seurat_nonEC_advSC.merged <- AddMetaData(object = seurat_nonEC_advSC.merged,
                                         metadata = percent.mito, col.name = "percent.mito")
VlnPlot(object = seurat_nonEC_advSC.merged, features.plot = c("nGene", "nUMI", "percent.mito"), nCol = 3)
par(mfrow = c(1, 2))
GenePlot(object = seurat_nonEC_advSC.merged, gene1 = "nUMI", gene2 = "percent.mito")
GenePlot(object = seurat_nonEC_advSC.merged, gene1 = "nUMI", gene2 = "nGene")
#Filter cells
seurat_nonEC_advSC.merged <- FilterCells(object = seurat_nonEC_advSC.merged, subset.names = c("nGene", "percent.mito"), 
                                     low.thresholds = c(min_gene_number, -Inf), high.thresholds = c(nGene, mito_cutoff),
                                     cells.use = seurat_nonEC_SC.integrated_ONLY_Adventitial)
#Number of cells
print(paste("Number of non-endothelial SC: ", nrow(seurat_nonEC_advSC.merged@meta.data), sep = ""))

#Normalize
seurat_nonEC_advSC.merged <- NormalizeData(seurat_nonEC_advSC.merged)
seurat_nonEC_advSC.merged <- FindVariableGenes(seurat_nonEC_advSC.merged, do.plot = F, display.progress = F)
seurat_nonEC_advSC.merged <- ScaleData(seurat_nonEC_advSC.merged)
#Run PCA
seurat_nonEC_advSC.merged <- RunPCA(object = seurat_nonEC_advSC.merged, pc.genes = Tx_LN_nonEC_advSC.merged@var.genes, do.print = TRUE, pcs.print = 1:10, 
                                genes.print = 10)

# Examine and visualize PCA results
PrintPCA(object = seurat_nonEC_advSC.merged, pcs.print = 1:5, genes.print = 5, use.full = FALSE)
VizPCA(object = seurat_nonEC_advSC.merged, pcs.use = 1:6)
PCAPlot(object = seurat_nonEC_advSC.merged, dim.1 = 1, dim.2 = 2)
...
# Run multi-set CCA
seurat_nonEC_advSC.integrated <- RunMultiCCA(l_ob_nonEC_advSC, genes.use = genes.use, num.ccs = dims_use)

# CC Selection
MetageneBicorPlot(seurat_nonEC_advSC.integrated, grouping.var = "tech", dims.eval = 1:15)

# Run rare non-overlapping filtering
seurat_nonEC_advSC.integrated <- CalcVarExpRatio(object = seurat_nonEC_advSC.integrated, reduction.type = "pca",
                                             grouping.var = "tech", dims.use = 1:8)
seurat_nonEC_advSC.integrated <- SubsetData(seurat_nonEC_advSC.integrated, subset.name = "var.ratio.pca",
                                        accept.low = 0.5)

# Alignment
seurat_nonEC_advSC.integrated <- AlignSubspace(seurat_nonEC_advSC.integrated,
                                           reduction.type = "cca",
                                           grouping.var = "tech",
                                           dims.align = 1:12)

# t-SNE and Clustering
seurat_nonEC_advSC.integrated <- FindClusters(seurat_nonEC_advSC.integrated, reduction.type = "cca",
                                          dims.use = 1:16, save.SNN = T, resolution = 0.8)
seurat_nonEC_advSC.integrated <- RunTSNE(seurat_nonEC_advSC.integrated,
                                     reduction.use = "cca",
                                     dims.use = 1:16, save.SNN = T,
                                     resolution = resolution_merge, force.recalc = TRUE)
...
#Check allocation of cells
#sample_1_cells <- grep(pattern = "^sample_1_", x = rownames(seurat_nonEC_advSC.integrated@meta.data), value = TRUE)
sample_2_cells <- grep(pattern = "^sample_2_", x = rownames(seurat_nonEC_advSC.integrated@meta.data), value = TRUE)
sample_3_cells <- grep(pattern = "^sample_3_", x = rownames(seurat_nonEC_advSC.integrated@meta.data), value = TRUE)
sample_4_cells <- grep(pattern = "^sample_4_", x = rownames(seurat_nonEC_advSC.integrated@meta.data), value = TRUE)
#p1 <- TSNEPlot(seurat_nonEC_advSC.integrated, group.by = "tech", do.return = T, pt.size = 0.4, cells.use = sample_1_cells)
p2 <- TSNEPlot(seurat_nonEC_advSC.integrated, group.by = "tech", do.return = T, pt.size = 0.4, cells.use = sample_2_cells)
p3 <- TSNEPlot(seurat_nonEC_advSC.integrated, group.by = "tech", do.return = T, pt.size = 0.4, cells.use = sample_3_cells)
p4 <- TSNEPlot(seurat_nonEC_advSC.integrated, group.by = "tech", do.return = T, pt.size = 0.4, cells.use = sample_4_cells)
p5 <- TSNEPlot(seurat_nonEC_advSC.integrated, do.return = T, pt.size = 0.2, do.label = FALSE)
#test <- LN@meta.data[order(-LN@meta.data$),]
p6 <- TSNEPlot(seurat_nonEC_advSC.integrated, group.by = "tech", do.return = T, pt.size = 0.2,
               colors.use = c("orange","green","blue","purple"))
par(mfrow = c(3, 2))
plot_grid(p2, p3, p4, p5, p6)


FeaturePlot(seurat_nonEC_advSC.integrated, features.plot = c("Pdpn","Pecam1","Lyve1","Ackr4", 
                                                          "Tnfsf11","Aldh1a3","Ptgis","Ccl19",
                                                          "Madcam1","Cd34","Tnfsf13b","Il6", 
                                                          "Il6","Ackr3","Ly6c1","Col4a1"),
            min.cutoff = "q9",
...
FeaturePlot(seurat_nonEC_advSC.integrated, features.plot = c("Tnfsf11"),
            min.cutoff = "q9",
            cols.use = c('lightgrey', 'brown'),
            pt.size = 0.2)

#Save Seurat Object
saveRDS(seurat_nonEC_advSC.integrated, file=paste(output_PATH_RDS,"/seurat_nonEC_advSC.integrated.Rds",sep=""))
seurat_nonEC_advSC.integrated <- readRDS(file=paste(output_PATH_RDS,"/seurat_nonEC_advSC.integrated.Rds",sep=""))
#####
