...
# 2) Generate BED for Peaks present across all replicates

#Libraries
library("GenomicRanges")

# Input required: Set path to directory with the MACS2 XLS files
path_input <- setwd("/home/pezoldt/NAS2/pezoldt/Analysis/ATACseq/ATAC_FSC_all/peaks/broad/MACS2_broad_XLS")
path_input_blacklist <- "/home/pezoldt/NAS2/pezoldt/Analysis/ATACseq/ATAC_FSC_all/peaks/broad/mm10.blacklist_kundaje_20180718_final.bed"
path_output <- "/home/pezoldt/NAS2/pezoldt/Analysis/ATACseq/ATAC_FSC_all/peaks/broad/MACS2_broad/Run_3"
# Input required: add group names that allow to distinguish experimental conditions across replicates
experimental_groups <- c("ATAC_mLNGF","ATAC_mLNSPF","ATAC_pLNGF","ATAC_pLNSPF")
...
#####
#3) Identify Peaks present in all files
#####
...
for(i in seq(length(experimental_groups))){
  ...
  for(k in seq(length(grange_obj_list_j)-1)){
    ...
    #find overlap
    overlap_index <- suppressWarnings(findOverlaps(gr_final, gr_index, minoverlap = 25))
    #get index numbers for overlapping peaks
    query_overlap <- queryHits(overlap_index)
    #get Granges object for overlapping peaks
...
#Take output and pipe it into "JP_atac_merge_peaks_enrichment.bash"
