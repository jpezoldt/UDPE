...
for(i in seq(length(t_Prom_Feat_TF_reg))){
  ...
  write.table(t_Prom_Feat_TF_reg_i, paste(path_output_Motif, "/", name_i,"_noExtension50bp.txt", sep = ""),
...
for(i in seq(length(t_Prom_Feat_TF_act))){
  ...
  print(name_i)
  rownames(t_Prom_Feat_TF_act_i) <- c()
  write.table(t_Prom_Feat_TF_act_i, paste(path_output_Motif, "/", name_i,"_noExtension50bp.txt", sep = ""),
              row.names = FALSE,quote=FALSE,sep="\t")
}
...
for(i in seq(length(TF_DARs))){
  ...
  write.table(DARs_features_TSS_bed_i, file=paste(path_output_Motif, "/", names(TF_DARs)[i],"_noExtension50bp.bed", sep = ""),
...
for(i in seq(length(TF_Activity))){
  ...
  write.table(PotAct_features_TSS_bed_i, file=paste(path_output_Motif, "/", names(TF_Activity)[i],"_noExtension50bp.bed", sep = ""),
              quote=F, sep="\t", row.names=F, col.names=F)
}
...
write.table(background_TSS, file=paste(path_output_Motif, "/background_all_TSS_noExtension50bp_minus_input_genes",".bed", sep = ""),
            quote=F, sep="\t", row.names=F, col.names=F)
...
for(i in 1:length(List_allRes)){
  ...
  table_tophits <- subset(table_i, weight < 0.001)
  #print(i)
