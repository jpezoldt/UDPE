...
plot_cell_clusters(cde_all, 1, 2, color = "Cluster", markers = c("Pecam1","Cd34","Tnfsf11","Aldh1a2","Cxcl13",
                                                                ...
                                                                "Cd248","Ackr3","Cxcl13"),cell_size = 0.5)
plot_cell_clusters(cde_all, 1, 2, color = "Cluster") + facet_wrap(~Time_point)

#Save RDS
saveRDS(cde_all, file=paste(PATH_CDS_objects,condition,"/CDS_objects","/day0_10_24_56_300_ALL.Rds",sep=""))
...
#Save RDS
saveRDS(cde_Endothelial, file=paste(PATH_CDS_objects,condition,"/CDS_objects","/day0_10_24_56_300_Endothelial.Rds",sep=""))
...
plot_cell_clusters(cde_SC, 1, 2, color = "Cluster", markers = c("Cd34","Tnfsf11","Aldh1a2","Cxcl13",
                                                                "Nfkb1","Ltbr","Icam1","Vcam1","Acta2",
                                                                "Cd248","Ackr3","Il21"),cell_size = 0.5)
...
#Save RDS
saveRDS(cde_SC, file=paste(PATH_CDS_objects,condition,"/CDS_objects","/day0_10_24_56_300_SC.Rds",sep=""))
...
for(j in 1:length(PvC.genes)){
  PvC.genes_j <- PvC.genes[1]
  for(i in seq(length(levels(pData(cde_SC)$Cluster)))){
    cluster_ID_i <- levels(pData(cde_SC)$Cluster)[1]
    print(cluster_ID_i)
...
cde_nonPvC <- reduceDimension(cde_nonPvC, max_components = 2, num_dim = 10,
                              ...
                              residualModelFormulaStr = "~Total_mRNAs + mito_expression + ribo_expression + Time_point",
                              verbose = T)
cde_nonPvC <- clusterCells(cde_nonPvC, num_clusters = 12)
...
plot_cell_trajectory(cde_nonPvC, color_by = "State", markers = c("Cxcl13"), use_color_gradient = TRUE)  + facet_wrap(~Time_point)
plot_cell_trajectory(cde_nonPvC, color_by = "State", markers = c("Cd34"), use_color_gradient = TRUE) + facet_wrap(~Time_point)

plot_complex_cell_trajectory(cde_nonPvC, color_by = 'State', show_branch_points = T,
                             cell_size = 0.5, cell_link_size = 0.3, root_states = c(2))

#Save RDS
saveRDS(cde_nonPvC, file=paste(PATH_CDS_objects,condition,"/CDS_objects","/day0_10_24_56_300_nonPvC.Rds",sep=""))

#####
#Signature identification across subsets
#####
cde_to_check <- cde_nonLTO
...
pdf(paste(PATH_CDS_objects,condition,"/day0_10_24_56_300_nonLTO_NC_clusters.pdf", sep=""))
