...
#PATH
#####
output_PATH_RDS <- "/home/pezoldt/NAS2/pezoldt/Analysis/scRNAseq/seurat/Tx_LN/RDS"
output_PATH_Figures <- "/home/pezoldt/NAS2/pezoldt/Analysis/scRNAseq/seurat/Tx_LN/Figures"
...
setwd("/home/pezoldt/NAS2/pezoldt/Analysis/scRNAseq/seurat/Tx_LN")
...
Tx_LN.merged <- MergeSeurat(sample_1_seurat, sample_2_seurat, do.normalize = FALSE)
Tx_LN.merged <- MergeSeurat(Tx_LN.merged, sample_3_seurat, do.normalize = FALSE)
Tx_LN.merged <- MergeSeurat(Tx_LN.merged, sample_4_seurat, do.normalize = FALSE)
Tx_LN.merged <- NormalizeData(Tx_LN.merged)
Tx_LN.merged <- FindVariableGenes(Tx_LN.merged, do.plot = F, display.progress = F)
Tx_LN.merged <- ScaleData(Tx_LN.merged)
#Run PCA
Tx_LN.merged <- RunPCA(object = Tx_LN.merged, pc.genes = Tx_LN.merged@var.genes, do.print = TRUE, pcs.print = 1:10, 
                          genes.print = 10)

# Examine and visualize PCA results
PrintPCA(object = Tx_LN.merged, pcs.print = 1:5, genes.print = 5, use.full = FALSE)
VizPCA(object = Tx_LN.merged, pcs.use = 1:6)
PCAPlot(object = Tx_LN.merged, dim.1 = 1, dim.2 = 2)
...
# Run multi-set CCA
Tx_LN.integrated <- RunMultiCCA(ob.list, genes.use = genes.use, num.ccs = dims_use)

# CC Selection
MetageneBicorPlot(Tx_LN.integrated, grouping.var = "tech", dims.eval = 1:15)

# Run rare non-overlapping filtering
Tx_LN.integrated <- CalcVarExpRatio(object = Tx_LN.integrated, reduction.type = "pca",
                                       grouping.var = "tech", dims.use = 1:dims_use)
Tx_LN.integrated <- SubsetData(Tx_LN.integrated, subset.name = "var.ratio.pca",
                                  accept.low = 0.5)

# Alignment
Tx_LN.integrated <- AlignSubspace(Tx_LN.integrated,
                                     reduction.type = "cca",
                                     grouping.var = "tech",
                                     dims.align = 1:16)

#Plot the aligned CCA ACCA
p1 <- VlnPlot(Tx_LN.integrated, features.plot = "ACC1", group.by = "tech", do.return = T)
p2 <- VlnPlot(Tx_LN.integrated, features.plot = "ACC2", group.by = "tech", do.return = T)
plot_grid(p1, p2)

# t-SNE and Clustering
Tx_LN.integrated <- FindClusters(Tx_LN.integrated, reduction.type = "cca",
                                    dims.use = 1:16, save.SNN = T, resolution = 0.8)
Tx_LN.integrated <- RunTSNE(Tx_LN.integrated,
                               reduction.use = "cca",
...
sample_1_cells <- grep(pattern = "^sample_1_", x = rownames(Tx_LN.integrated@meta.data), value = TRUE)
sample_2_cells <- grep(pattern = "^sample_2_", x = rownames(Tx_LN.integrated@meta.data), value = TRUE)
sample_3_cells <- grep(pattern = "^sample_3_", x = rownames(Tx_LN.integrated@meta.data), value = TRUE)
sample_4_cells <- grep(pattern = "^sample_4_", x = rownames(Tx_LN.integrated@meta.data), value = TRUE)
p1 <- TSNEPlot(Tx_LN.integrated, group.by = "tech", do.return = T, pt.size = 0.4, cells.use = sample_1_cells)
p2 <- TSNEPlot(Tx_LN.integrated, group.by = "tech", do.return = T, pt.size = 0.4, cells.use = sample_2_cells)
p3 <- TSNEPlot(Tx_LN.integrated, group.by = "tech", do.return = T, pt.size = 0.4, cells.use = sample_3_cells)
p4 <- TSNEPlot(Tx_LN.integrated, group.by = "tech", do.return = T, pt.size = 0.4, cells.use = sample_4_cells)
p5 <- TSNEPlot(Tx_LN.integrated, do.return = T, pt.size = 0.2, do.label = FALSE)
#test <- LN@meta.data[order(-LN@meta.data$),]
p6 <- TSNEPlot(Tx_LN.integrated, group.by = "tech", do.return = T, pt.size = 0.2,
               colors.use = c("orange","green","blue","purple"))
par(mfrow = c(3, 2))
plot_grid(p1, p2, p3, p4, p5, p6)


FeaturePlot(Tx_LN.integrated, features.plot = c("Pdpn","Pecam1","Lyve1","Ackr4", 
...
#Save Seurat Object
saveRDS(Tx_LN.integrated, file=paste(output_PATH_RDS,"/Tx_LN.integrated.Rds",sep=""))

#####
#Select non-endothelial SCs
#####
#Identify LECs and BECs
VlnPlot(object = Tx_LN.integrated, features.plot = c("Pecam1"))
Tx_LN.integrated_aExp <- AverageExpression(Tx_LN.integrated)
aExp_Pecam1 <- subset(Tx_LN.integrated_aExp, rownames(Tx_LN.integrated_aExp) == c("Pecam1"))
...
all_cells <- Tx_LN.integrated@meta.data
...
Tx_LN.integrated_NO_LECs_BECs <- as.character(rownames(all_cells_keep))
