...
data_heatmap[data_heatmap == -Inf] <- -8
min(data_heatmap)
data_heatmap_matrix <- data.matrix(data_heatmap)
...
expression <- data_all[,c("GeneSymbol",
                          ...
                          "log2FC_SPF_Treg_VS_GF_Treg","log2FC_SPF_Treg_VS_SPF_Tconv",
                          "log2FC_SPF_Treg_VS_Naive_T","log2FC_SPF_Tconv_VS_Naive_T",
                          "padj_GF_Treg_VS_GF_Tconv","padj_SPF_Tconv_VS_GF_Tconv",
                          "padj_SPF_Treg_VS_GF_Treg","padj_SPF_Treg_VS_SPF_Tconv",
                          "padj_SPF_Treg_VS_Naive_T","padj_SPF_Tconv_VS_Naive_T")]

print(paste("Number of expressed genes across comparison:",nrow(expression[!is.na(expression$GeneSymbol),])))
colnames(expression) <- c("GeneSymbol",
                          "exp_log2FC_GF_Treg_VS_GF_Tconv","exp_log2FC_SPF_Tconv_VS_GF_Tconv",
                          "exp_log2FC_SPF_Treg_VS_GF_Treg","exp_log2FC_SPF_Treg_VS_SPF_Tconv",
                          "exp_log2FC_SPF_Treg_VS_Naive_T","exp_log2FC_SPF_Tconv_VS_Naive_T",
                          "exp_padj_GF_Treg_VS_GF_Tconv","exp_padj_SPF_Tconv_VS_GF_Tconv",
                          "exp_padj_SPF_Treg_VS_GF_Treg","exp_padj_SPF_Treg_VS_SPF_Tconv",
                          "exp_padj_SPF_Treg_VS_Naive_T","exp_padj_SPF_Tconv_VS_Naive_T")
...
DARs_features_NA_sig <- subset(DARs_features_NA, abs(log2FC_Tnaive_SPF_Treg) >= 1.0 & padj_Tnaive_SPF_Treg <= 0.05)
...
#mean_FC_per_DAR <- lapply(l_DARs_features_NA, function(x){
 # FC_mean <- mean(x$log2FC_SPF_mLN_pLN)
 # FC_mean
#})
...
plot(DARs_DEGs$log2FC_Tnaive_SPF_Treg, DARs_DEGs$exp_log2FC_SPF_Treg_VS_Naive_T)
#Change orientation of FC for expression
DARs_DEGs$log2FC_Tnaive_SPF_Treg <- DARs_DEGs$log2FC_Tnaive_SPF_Treg * (-1)
plot(DARs_DEGs$log2FC_Tnaive_SPF_Treg, DARs_DEGs$exp_log2FC_SPF_Treg_VS_Naive_T)

#Set Quadrants
#####
#mLN open and UP - DEG regulated by openess
right_up <- as.numeric(DARs_DEGs$log2FC_Tnaive_SPF_Treg > 1.0 &
                         DARs_DEGs$exp_log2FC_SPF_Treg_VS_Naive_T > 1.0 &
                         (DARs_DEGs$padj_Tnaive_SPF_Treg < 0.05 |
                            DARs_DEGs$exp_padj_SPF_Treg_VS_Naive_T < 0.05))
right_up_n_Genes <- unique(subset(DARs_DEGs, DARs_DEGs$log2FC_Tnaive_SPF_Treg > 1.0 &
                                    DARs_DEGs$exp_log2FC_SPF_Treg_VS_Naive_T > 1.0 &
                                    (DARs_DEGs$padj_Tnaive_SPF_Treg < 0.05 |
                                       DARs_DEGs$exp_padj_SPF_Treg_VS_Naive_T < 0.05))$symbol)

#pLN open and UP - - DEG regulated by openess
left_down <- as.numeric(DARs_DEGs$log2FC_Tnaive_SPF_Treg < -1.0 &
                          DARs_DEGs$exp_log2FC_SPF_Treg_VS_Naive_T < -1.0 &
                          (DARs_DEGs$padj_Tnaive_SPF_Treg < 0.05 |
                             DARs_DEGs$exp_padj_SPF_Treg_VS_Naive_T < 0.05))
left_down_n_Genes <- unique(subset(DARs_DEGs, DARs_DEGs$log2FC_Tnaive_SPF_Treg < -1.0 &
                                     DARs_DEGs$exp_log2FC_SPF_Treg_VS_Naive_T < -1.0 &
                                     (DARs_DEGs$padj_Tnaive_SPF_Treg < 0.05 |
                                        DARs_DEGs$exp_padj_SPF_Treg_VS_Naive_T < 0.05))$symbol)
...
#####Continue here XXX !!!
right <- as.numeric(DARs_DEGs$log2FC_Tnaive_SPF_Treg > 1.0 &
                      DARs_DEGs$exp_log2FC_SPF_Treg_VS_Naive_T > -1.0 &
                      DARs_DEGs$exp_log2FC_SPF_Treg_VS_Naive_T < 1.0 &
                      DARs_DEGs$padj_Tnaive_SPF_Treg < 0.05)
right_n_Genes <- unique(subset(DARs_DEGs, DARs_DEGs$log2FC_Tnaive_SPF_Treg > 1.0 &
                                 DARs_DEGs$exp_log2FC_SPF_Treg_VS_Naive_T > -1.0 &
                                 DARs_DEGs$exp_log2FC_SPF_Treg_VS_Naive_T < 1.0 &
                                 DARs_DEGs$padj_Tnaive_SPF_Treg < 0.05)$symbol)
right <- replace(right, right == 1, 3)

#pLN open no DEG - activatable
left <- as.numeric(DARs_DEGs$log2FC_Tnaive_SPF_Treg < -1.0 &
                     DARs_DEGs$exp_log2FC_SPF_Treg_VS_Naive_T > -1.0 &
                     DARs_DEGs$exp_log2FC_SPF_Treg_VS_Naive_T < 1.0 &
                     DARs_DEGs$padj_Tnaive_SPF_Treg < 0.05)
left_n_Genes <- unique(subset(DARs_DEGs, DARs_DEGs$log2FC_Tnaive_SPF_Treg < -1.0 &
                                DARs_DEGs$exp_log2FC_SPF_Treg_VS_Naive_T > -1.0 &
                                DARs_DEGs$exp_log2FC_SPF_Treg_VS_Naive_T < 1.0 &
                                DARs_DEGs$padj_Tnaive_SPF_Treg < 0.05)$symbol)
...
top <- as.numeric(DARs_DEGs$exp_log2FC_SPF_Treg_VS_Naive_T > 1.0 &
                    DARs_DEGs$log2FC_Tnaive_SPF_Treg > -1.0 &
                    DARs_DEGs$log2FC_Tnaive_SPF_Treg < 1.0 &
                    DARs_DEGs$exp_padj_SPF_Treg_VS_Naive_T < 0.05)
top_n_Genes <- unique(subset(DARs_DEGs,DARs_DEGs$exp_log2FC_SPF_Treg_VS_Naive_T > 1.0 &
                               DARs_DEGs$log2FC_Tnaive_SPF_Treg > -1.0 &
                               DARs_DEGs$log2FC_Tnaive_SPF_Treg < 1.0 &
                               DARs_DEGs$exp_padj_SPF_Treg_VS_Naive_T < 0.05)$symbol)
...
bottom <- as.numeric(DARs_DEGs$exp_log2FC_SPF_Treg_VS_Naive_T < -1.0 &
                       DARs_DEGs$log2FC_Tnaive_SPF_Treg > -1.0 &
                       DARs_DEGs$log2FC_Tnaive_SPF_Treg < 1.0 &
                       DARs_DEGs$exp_padj_SPF_Treg_VS_Naive_T < 0.05)
bottom_n_Genes <- unique(subset(DARs_DEGs,DARs_DEGs$exp_log2FC_SPF_Treg_VS_Naive_T < -1.0 &
                                  DARs_DEGs$log2FC_Tnaive_SPF_Treg > -1.0 &
                                  DARs_DEGs$log2FC_Tnaive_SPF_Treg < 1.0 &
                                  DARs_DEGs$exp_padj_SPF_Treg_VS_Naive_T < 0.05)$symbol)
...
#Plot
d <- ggplot(data_DEGs_DARs, aes(log2FC_Tnaive_SPF_Treg, exp_log2FC_SPF_Treg_VS_Naive_T)) #, colour = factor(ordering))
d + geom_point(aes(colour = factor(ordering)), size = 2.0, show_guide = FALSE) +
  ...
  #Numbers
  geom_text(data = data.frame(), aes(7, 8, label = right_up_n), colour = "gray48", size = 8) + 
  geom_text(data = data.frame(), aes(-5, -5, label = left_down_n), colour = "gray32", size = 8) +
  geom_text(data = data.frame(), aes(-5, 0, label = left_n), colour = "deeppink3", size = 8) + 
  geom_text(data = data.frame(), aes(7, 0, label = right_n), colour = "deepskyblue", size = 8) +
  geom_text(data = data.frame(), aes(0, 8, label = top_n), colour = "deepskyblue4", size = 8) + 
  geom_text(data = data.frame(), aes(0, -5, label = bottom_n), colour = "deeppink4", size = 8) +
  
  #Lab
  xlab("ATAC log(FC) Tconv/Tnaive") +
  ylab("RNAseq log(FC) Tconv/Tnaive") +
