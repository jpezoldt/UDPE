...
for(i in seq(length(TF_DARs))){
  ...
  write.table(DARs_features_TSS_bed_i, file=paste(path_output_Motif, "/", names(TF_DARs)[i],"_Ext50bp.bed", sep = ""),
...
for(i in seq(length(TF_Activity))){
  ...
  write.table(PotAct_features_TSS_bed_i, file=paste(path_output_Motif, "/", names(TF_Activity)[i],"_Ext.bed", sep = ""),
              quote=F, sep="\t", row.names=F, col.names=F)
}
...
write.table(background, file=paste(path_output_Motif, "/background_all_peaks_minus_input_Ext50bp",".bed", sep = ""),
            quote=F, sep="\t", row.names=F, col.names=F)
